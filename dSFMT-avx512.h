...
inline static void do_recursion_x4_avx512(w128_t* r, w128_t* a, w128_t* b, __m512i* lung)
{
    ...
    t = _mm512_inserti64x4(_mm512_setzero_si512(), _mm512_castsi512_si256(z), 1);
    v = _mm512_maskz_permutexvar_epi32(shift384_reverse_mask,shift384_reverse_idx , z);
    z = _mm512_ternarylogic_epi32(z, w, t, 0x96);

    y = expand_lung(*lung, 4);
...
void gen_rand_array(dsfmt_t *dsfmt, w128_t *array,
    ...
    if (i < loop1_end) {
        do_recursion_x4_avx512_frac(&array[i], &dsfmt->status[i], &dsfmt->status[i + DSFMT_POS1], &lung, loop1_end - i);

    }

    const ptrdiff_t loop2_end = DSFMT_N;
    for (; i <= loop2_end - 4; i += 4) {
        do_recursion_x4_avx512(&array[i], &dsfmt->status[i], &array[i + DSFMT_POS1 - DSFMT_N], &lung);
    }
    if (i < loop2_end) {
        do_recursion_x4_avx512_frac(&array[i], &dsfmt->status[i], &array[i + DSFMT_POS1 - DSFMT_N], &lung, loop2_end-i);
        i = loop2_end;
    }

    const ptrdiff_t loop3_end = size - DSFMT_N;
    for (; i <= loop3_end - 4; i += 4) {
        do_recursion_x4_avx512(&array[i], &array[i - DSFMT_N], &array[i + DSFMT_POS1 - DSFMT_N], &lung);
    }
    if (i < loop3_end) {
        do_recursion_x4_avx512_frac(&array[i], &array[i - DSFMT_N], &array[i + DSFMT_POS1 - DSFMT_N], &lung, loop3_end-i);
        i= loop3_end;
    }
    ...
    if (i < loop5_end) {
        do_recursion_x4_avx512_frac(&array[i], &array[i - DSFMT_N], &array[i + DSFMT_POS1 - DSFMT_N], &lung, loop5_end-i);
        for (; i < loop5_end; i++,j++) {
...
inline static void dsfmt_gen_rand_all_impl(dsfmt_t *dsfmt) {
    ...
    if (i < loop1_end) {
        ...
        do_recursion_x4_avx512_frac(&dsfmt->status[i], &dsfmt->status[i], &dsfmt->status[i + DSFMT_POS1], &lung, nCount);
    }
    ...
    if ( i < loop2_end) {
        ...
        do_recursion_x4_avx512_frac(&dsfmt->status[i], &dsfmt->status[i], &dsfmt->status[i + DSFMT_POS1 - DSFMT_N], &lung, nCount);
    }
