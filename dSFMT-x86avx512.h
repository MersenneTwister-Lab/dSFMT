...
#ifndef DSFMT_X86AVX512_H
#define DSFMT_X86AVX512_H
...
inline static __m512i do_recursion_x4_avx512(
    __m512i a, __m512i b, __m512i* u, const int nLane
) {
    const __mmask output_mask = (__mmask16)((1 << 4*nLane)-1);
    ...
    __mmask16 lane0clear = 0xFFF0;
    x = _mm512_maskz_permutexvar_epi32(lane0clear,shift128_reverse_idx , z);
    z = _mm512_xor_si512(z, x);

    x = mm512_bsrli_epi128(z, 32); 

    if (nLane == 4) {
        w = *u;
        y = _mm512_permutexvar_epi32(expand_lung_idx, w);  /* lung */
        *w = _mm512_ternarylogic_epi32(w, x, z, 0x96);
        y = _mm512_ternarylogic_epi32(y, x, z, 0x96);
    } else {
        y = _mm512_maskz_ternarylogic_epi32(y, x, z, 0x96);
