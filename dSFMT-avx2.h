...
inline static void gen_rand_array_avx_core(dsfmt_t *dsfmt, w128_t *array,
    ...
    for (; i < loop3_end; i += 2) {
        __m256i result = do_recursion_x2_avx2(
            ...
            &lung);
        ST256(&array[i], result);
        converter((__m256d*)&array[i - DSFMT_N]);
    }
    ...
    for (i = loop3_end; i < size; i+= 2) {
        converter((__m256d*)&array[i]);
    }
    if (size & 1) {
        __m256d data = _mm256_castpd128_pd256(_mm_loadu_pd((double*)&array[i]));
        converter(&data);
...
void dsfmt_gen_rand_all(dsfmt_t *dsfmt) {
    ...
    if (DSFMT_N & 1) {
        __m128i lung128 = _mm256_extracti128_si256(lung, 1);
        __m128i result = do_recursion_last(
            LD128(&dsfmt->status[DSFMT_N-1]), 
            LD128(&dsfmt->status[i + DSFMT_POS1 - DSFMT_N]),
            &lung128);
